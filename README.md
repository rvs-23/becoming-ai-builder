...
We work in **arcs**. An arc is a few weeks on one mission that ends in something we built and can show off. There are eight, and each one becomes a folder in the repo (`arc-1`, `arc-2`, and so on). We pick a theme we care about: games, basketball, finance, whatever's fun.
